...
logging.basicConfig(filename='P4_output.log', 
                    level=logging.INFO, 
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Opcional: también mostrar logs en la consola
...
def check_and_create_directory(directory_path):
    if not os.path.exists(directory_path):
        ...
        logging.info(f"The directory '{directory_path}' has been created.")
    else:
        logging.info(f"The directory '{directory_path}' already exists.")


MODEL_NAME = "nonlinear_problem_P4"
...
class NeuralNetwork(tf.keras.Model):
    ...
    def __init__(self, input_size, hidden1_dim, hidden2_dim, output_size, n_filters=5, **kwargs):
        ...
        # Explanatory network
        self.conv_expand_layer = tf.keras.layers.Conv2D(self.n_filters, (1, 1), activation=None, name="conv_expand_layer")
        self.hidden_layer_1 = tf.keras.layers.Dense(hidden1_dim, activation='sigmoid', name="hidden_layer_1")
        self.hidden_layer_2 = tf.keras.layers.Dense(self.n_filters, activation='sigmoid', name="hidden_layer_2")
        self.conv_converge_layer = tf.keras.layers.Conv2D(1, (1, 1), activation=None, name="conv_converge_layer")
    ...
    def call(self, X):
        ...
        X = tf.expand_dims(um_pred, axis=-1)
        batch_size, height, width, channels = tf.shape(X)[0], tf.shape(X)[1], tf.shape(X)[2], tf.shape(X)[3]
        X = self.conv_expand_layer(X)  # (batch, height, width, n_filters)        
        X = tf.reshape(X, (batch_size * height * width, self.n_filters))  # Flatten spatial dimensions

        X = self.hidden_layer_1(X)
        X = self.hidden_layer_2(X)
        X = tf.reshape(X, (batch_size, height, width, self.n_filters))
        K_pred = tf.squeeze(self.conv_converge_layer(X), axis=-1)
    ...
    with tf.device(device):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % (1 if num_epochs < 100 else (10 if num_epochs <= 1000 else 100)) == 0:
                ...
                logging.info(f"Epoch {epoch}, Memory Usage: {memory_usage_mb:.2f} MB")
                logging.info(f'Epoch {epoch}, Train loss: {loss.numpy()}, Test loss: {test_loss.numpy()}, sum(MSE) train: {sum_MSE_train.numpy()}, sum(MSE) test: {sum_MSE_test.numpy()}, MSE(e): {MSE(e)}, MSE(pi1): {MSE(pi1)}, MSE(pi2): {MSE(pi2)}, MSE(pi3): {MSE(pi3)}')
...
if __name__ == "__main__":
    ...
    args.model_name = "nonlinear_problem_P4" #ESTO ESTA PUESTO PROVISIONALMENTE PARA QUE NO DE PROBLEMAS. LA IDEA DE ESTO ES PASARLO COMO ARGUMENTO DEL PROGRAMA

    logging.info("\n")
    logging.info("Python script configuration:")
    logging.info(f" Epochs of training: {args.epochs}")
    logging.info(f" Resume training (0 == No, 1 == Yes): {args.resume_training}")
    ...
    logging.info("\n")
    logging.info("Neural network shape")
    logging.info("----------------------")
    logging.info(f"Neurons in the input layer: {input_shape}")
    logging.info(f"Neurons in the 1st hidden layer: {hidden1_dim}")
    logging.info(f"Neurons in the 2nd hidden layer: {hidden2_dim}")
    logging.info(f"Neurons in the output layer: {output_shape}")
    logging.info("\n")
    ...
    if args.resume_training and args.model_name:
        ...
        logging.info(f"Execution time of train_neural_network: {execution_time} seconds.")
        ...
        logging.info("\n")
        logging.info("Training of the model from scratch.")
        logging.info("\n")
